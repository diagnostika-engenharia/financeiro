-- fin_ledger v3 (22/09/2026): saldo acumulado por conta (janela SUM OVER PARTITION BY conta ORDER BY data, id).
--   conta            = 'banco' | 'cofre' (parcelamento/previsto/inferência ficam com conta nula e saldo nulo)
--   saldo_acumulado  = reais. Cofre: soma corrida pura (inclui a linha "Saldo anterior", que segue fora da DRE).
--                      Banco: ancorado no saldo atual de fin_contas_bancarias (extrato não tem saldo de abertura):
--                      saldo_inicial = saldo_atual_conta − Σ(créditos − débitos) do extrato; o último ponto da série == saldo da conta.
-- A assinatura de retorno muda → precisa DROP antes do CREATE. fin_ledger_totais/fin_snapshot resolvem por nome e continuam válidas.
-- Restante idêntico a _migra_ledger_parcelas.sql (v2).
drop function if exists public.fin_ledger(text, text);
create function public.fin_ledger(p_competencia text default null, p_regime text default 'caixa')
returns table (
  ...
  observacao     text,
  condominio     text,
  conta          text,     -- 'banco' | 'cofre' | null
  saldo_acumulado numeric  -- reais; saldo da conta após este lançamento (ordem data, id)
)
language sql
stable
security definer
...
-- ── 1) banco ─────────────────────────────────────────────────────────
banco_ancora as (  -- saldo de abertura implícito = saldo atual das contas ativas − movimento total do extrato
  select round((select coalesce(sum(saldo),0) from public.fin_contas_bancarias where ativa)/100.0,2)
       - coalesce((select sum(case when tipo='credito' then valor else -valor end) from public.fin_transacoes_bancarias),0) saldo_ini
),
banco as (
  select 'banco'::text fonte, t.id::text ref_id, left(t.data::text,10) data,
         ...
         round(a.saldo_ini + sum(case when t.tipo='credito' then t.valor else -t.valor end)
               over (order by t.data, t.id rows between unbounded preceding and current row), 2) saldo_acumulado
  from public.fin_transacoes_bancarias t cross join banco_ancora a
),
-- ── 2) cofre (espelha _cofreComoTxns / _cofreCat / _cofreComp) ───────
cofre_raw as (   -- janela sobre TODAS as linhas (inclusive "Saldo anterior"); o filtro da DRE vem depois
  select c.*, lower(coalesce(c.descricao,'')||' '||coalesce(c.observacao,'')) txt,
         lower(coalesce(c.descricao,'')) d,
         round(sum(case when c.tipo='Entrada' then c.valor else -c.valor end)
               over (order by c.data, c.id rows between unbounded preceding and current row)/100.0, 2) saldo_acumulado
...
cofre as (
  select 'cofre'::text fonte, c.id::text ref_id, left(c.data::text,10) data,
         ...
         c.descricao, c.observacao, null::text condominio, c.saldo_acumulado
  from cofre_raw c
...
banco_final as (
  select case when ci.comp is not null then 'inferencia_carro' else 'banco' end fonte,
         ...
         b.condominio, b.saldo_acumulado
...
-- ── união ────────────────────────────────────────────────────────────
tudo as (
  select fonte, ref_id, data, competencia, tipo, tipo_categoria, categoria, valor, historico, descricao, observacao, condominio, 'banco'::text conta, saldo_acumulado from banco_final
  union all select fonte, ref_id, data, competencia, tipo, tipo_categoria, categoria, valor, historico, descricao, observacao, condominio, 'cofre'::text, saldo_acumulado from cofre
  union all select fonte, ref_id, data, competencia, tipo, tipo_categoria, categoria, valor, historico, descricao, observacao, condominio, null::text, null::numeric from parc
  union all select fonte, ref_id, data, competencia, tipo, tipo_categoria, categoria, valor, historico, descricao, observacao, condominio, null::text, null::numeric from prev where p_regime='competencia'
),
nat as (
...
select fonte, ref_id, data, competencia, tipo, natureza tipo_categoria, categoria, valor,
       ...
       historico, descricao, observacao, condominio, conta, saldo_acumulado
from nat
...
-- saldo por conta (último ponto da série do razão) — usado pelo app p/ cofre/pagamentos/pró-labore
create or replace function public.fin_saldos()
returns table (conta text, saldo numeric, ultima_data text, n bigint)
language sql stable security definer set search_path = public as $s$
  select conta, (array_agg(saldo_acumulado order by data desc, ref_id::bigint desc))[1] saldo, max(data) ultima_data, count(*) n
  from public.fin_ledger(null,'caixa') where conta is not null group by conta;
$s$;
grant execute on function public.fin_saldos() to service_role, authenticated;
